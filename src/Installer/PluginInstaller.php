...
class PluginInstaller extends LibraryInstaller
{
    ...
    public static function determinePlugins($packages, $pluginsDir = 'plugins', $vendorDir = 'vendor')
    {
        ...
        foreach ($packages as $package) {
            if ($package->getType() !== 'skinny-plugin') {
                continue;
            }
...
PHP;
     ...
     *
     * This installer only supports package of type 'skinny-plugin'.
     *
     * @return bool
     */
     ...
     *
     * After the plugin is installed, app's `skinny-plugins.php` config file is updated with
    ...
    public function updateConfig($name, $path)
    {
        ...
        if (!isset($config)) {
            $this->io->write(
                'ERROR - `vendor/skinny-plugins.php` file is invalid. ' .
    ...
    protected function ensureConfigFile($path)
    {
        ...
        if ($this->io->isVerbose()) {
            $this->io->write('Created vendor/skinny-plugins.php');
        }
    }
}
